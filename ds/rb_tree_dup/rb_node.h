...
bool dup_close(int tid, node_t** root)
{
	...
	for (auto& d : *duplications)
	{
		...
		if (dup->l && dup->l->p == orig)
			dup->l->p = dup;

		if (dup->r && dup->r->p == orig)
			dup->r->p = dup;
...
{
	if (do_print) std::cout << "~~constructor: new_node=" << this << std::endl;
		...
		if (child != NULL && allocated->find(child) == allocated->end() && allocated->find(orig_parent) == allocated->end())
		{
			if (node_parent_map->find(child) == node_parent_map->end())
			{
				...
				node_parent_map->insert({
						child, { child, orig_parent, idx, (unsigned short)(parent_height + 1) }});
			}
			else
			{
				if (node_parent_map->at(child).parent != orig_parent)
				{
					std::cout << "c=" << child << "; op=" << orig_parent << "; npmap->at(c)=" << node_parent_map->at(child).parent << std::endl;
					std::cout << ":( :(" << std::endl;
					exit(-1);
				}
			}
