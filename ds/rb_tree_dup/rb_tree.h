...
private:
    ...
    rb_node<skey_t, sval_t> * dup_epilogue(const int& tid, rb_node<skey_t, sval_t> * orig, rb_node<skey_t, sval_t> * dup)
    {
        ...
        if (do_insert)
        {
			if (do_print) std::cout << "~~duplications~~: orig=" << orig << "; dup=" << dup << std::endl;
	...
	void rotateLeft (const int & tid, rb_node<skey_t, sval_t> * x) 
	{
		if (do_print) std::cout << "rotateLeft(" << x << ")" << std::endl;
		...
		if (rl != NULL) { 
			auto rl_dup = dup_prologue(tid, rl);
			if (rl_dup != nullptr) {
				rl_dup->set_parent(x); 
				dup_epilogue(tid, rl, rl_dup);
			}
		} 
		...
		else if (xp->get_child(LEFT) == x->get_self()) {
		...
		r_dup = dup_prologue(tid, r);
		if (r_dup != nullptr) {
			r_dup->set_child(LEFT, x); 
			dup_epilogue(tid, r, r_dup);
		}

		x_dup = dup_prologue(tid, x);
		if (x_dup != nullptr) {
			x_dup->set_parent(r); 
			dup_epilogue(tid, x, x_dup);
		}
	...
	void rotateRight (const int & tid, rb_node<skey_t, sval_t> * x) 
	{
		if (do_print) std::cout << "rotateRight(" << x << ")" << std::endl;
		...
		if (lr != NULL) {
			auto lr_dup = dup_prologue(tid, lr);
			if (lr_dup != nullptr) {
				lr_dup->set_parent(x);
				dup_epilogue(tid, lr, lr_dup);
			}
		}
		...
		if (do_print) std::cout << "~~rotateRight~~: x=" << x << "; xp=" << xp << "; rxp=" << rxp << std::endl;
		if (xp == NULL) {
			auto root_dup = dup_prologue(tid, orig_root);
			if (root_dup != nullptr) {
				root_dup = l;
	...
	inline void setColor (const int & tid, rb_node<skey_t, sval_t> * n, int c) { 
		if (n != NULL) {
			...
			if (n_dup != nullptr) {
				n_dup->set_color(c); 
				if (do_print) std::cout << "~~setColor~~: n=" << n << "(" << n->c << "); n_dup=" << n_dup << "(" << n_dup->c << ")" << std::endl;
	...
	void fixAfterInsertion(const int & tid, rb_node<skey_t, sval_t> * x) {
		...
		while (x != NULL && x != new_root) { 
			...
			if (do_print) std::cout << "~~fixAfterInsertion~~: inside loop" << std::endl;

			auto px = parentOf(x);
			auto ppx = parentOf(parentOf(x));
			auto lppx = leftOf(ppx);

			if (do_print) std::cout << "~~fixAfterInsertion~~: x=" << x << "; px=" << px << "; ppx=" << ppx << "; lppx=" << lppx << std::endl;
			...
			if (px == lppx) {
				...
				if (colorOf(y) == RED) {
					if (do_print) std::cout << "1" << std::endl;
					...
					if (do_print) std::cout << "~~fixAfterInsertion~~: x=" << x << "; rpx=" << rpx <<  std::endl;
					if (x->get_self() == rpx) {
						x = parentOf(x);
						rotateLeft(tid, x);
					}
					...
					if (parentOf(parentOf(x)) != NULL)
						rotateRight(tid, parentOf(parentOf(x)));
				}
			} else {
				rb_node<skey_t, sval_t> * y = leftOf(parentOf(parentOf(x)));
				if (colorOf(y) == RED) {
					if (do_print) std::cout << "3" << std::endl;
					setColor(tid, parentOf(x), BLACK);
					setColor(tid, y, BLACK);
					setColor(tid, parentOf(parentOf(x)), RED);
					x = parentOf(parentOf(x));
				} else {
					if (do_print) std::cout << "4" << std::endl;
					...
					setColor(tid, parentOf(parentOf(x)), RED);
					if (parentOf(parentOf(x)) != NULL)
						rotateLeft(tid, parentOf(parentOf(x)));
				}
			}
		}
	...
	sval_t rb_dup_insert(const int & tid, skey_t Key, sval_t Val) {
		while (1)
        {
			if (Key == -1)
				...
				do_print = false;
			if (do_print) std::cout << "========================== " << Key << std::endl;
            auto guard = recmgr->getGuard(tid);
            dup_open<skey_t, sval_t>(tid, &root);
			if (do_print) print_tree();
