...
public:
    ...
    inline bool is_dup() const { return (flags & DUP_MASK) == DUP_MASK; } //tomer
	inline void set_dup() { flags ^= DUP_MASK; } //tomer
	inline bool is_del() const { return (flags & DEL_MASK) == DEL_MASK; }
...
thread_local node* cached_orig;

thread_local node* cached_dup;
...
{
    std::unordered_map<node*, bool> temp_locked(*locked);

	for (auto& l : temp_locked)
	{
		if (all || l.second)
        {
...
bool dup_close(int tid, node** root)
{
	...
	for (auto& d : *duplications)
	{
		...
		auto orig_idx = d.second.orig_idx;
        dup->set_dup(); //tomer
		orig->updated_self = dup;
        orig->original_self->updated_self = dup;
...
const Key& Innernode::key(size_t s) const {
    node * orig = (node*)this;
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return ((Innernode*)cached_dup)->slotkey[s];
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
            cached_orig = orig;
            cached_dup = found.dup;
...
bool Innernode::is_full() const {
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return (cached_dup->slotuse == btree_default_traits<Key, Value>::inner_slots);
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
bool Innernode::is_few() const {
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return (cached_dup->slotuse <= btree_default_traits<Key, Value>::inner_slots / 2);
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
node * Innernode::get_child(unsigned short slot) const 
{
    ...
    if (in_writing_function && dup_happened)
    {
        node * parent = (node*)this;
    
        if (cached_orig == parent)
        {
            parent = cached_dup;
            child = ((Innernode*)parent)->childid[slot];
        }
        if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(parent); it != duplications->end())
        {
            node * dup = it->second.dup;// (duplications->at(parent)).dup;
            cached_orig = parent;
            cached_dup = dup;
            parent = dup;
            child = ((Innernode*)parent)->childid[slot];
...
node ** Innernode::get_childid_vec() 
{
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return ((Innernode*)cached_dup)->childid;
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
Key Innernode::get_slotkey(unsigned short slot) const 
{
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return ((Innernode*)cached_dup)->slotkey[slot];
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
Key * Innernode::get_slotkey_vec() 
{
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return &((Innernode*)cached_dup)->slotkey[0];
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
const Key& Leafnode::key(size_t s) const {
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return ((Leafnode*)cached_dup)->slotdata[s].first;
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
bool Leafnode::is_full() const {
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return (cached_dup->slotuse == btree_default_traits<Key, Value>::leaf_slots);
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
bool Leafnode::is_few() const {
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return (cached_dup->slotuse <= btree_default_traits<Key, Value>::leaf_slots / 2);
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
Value Leafnode::get_slot(unsigned short slot) const 
{
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return ((Leafnode*)cached_dup)->slotdata[slot];
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
Value * Leafnode::get_slotdata_vec() 
{
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return &((Leafnode*)cached_dup)->slotdata[0];
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
...
Leafnode * Leafnode::get_next_leaf()
{
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == current)
        {
            return ((Leafnode*)cached_dup)->next_leaf;
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(current); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
            cached_orig = current;
            cached_dup = found.dup;
            return ((Leafnode*)found.dup)->next_leaf;
        }
...
Leafnode * Leafnode::get_prev_leaf()
{
    ...
    if (in_writing_function && dup_happened)// && duplications->find(orig) != duplications->end())
    {
        if (cached_orig == orig)
        {
            return ((Leafnode*)cached_dup)->prev_leaf;
        }
        else if (std::unordered_map<node*, duplication_info_t>::iterator it = duplications->find(orig); it != duplications->end())
        {
            auto found = it->second;//duplications->at(orig);
            cached_orig = orig;
            cached_dup = found.dup;
            return ((Leafnode*)found.dup)->prev_leaf;
        }
        // auto found = duplications->at(orig);
        // return ((Leafnode*)found.dup)->prev_leaf;
    }
