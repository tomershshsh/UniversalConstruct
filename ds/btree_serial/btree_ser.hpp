...
public:
    ...
    typedef tlx::BTree<key_type, value_type, key_of_value, key_compare,
...
public:
    ...
    struct btree_impl::node* get_root()
    {
        return tree_.root_;
    }
...
public:
    ...
    sval_t insert(const int tid, const skey_t& key, const sval_t& value) {
        ...
        auto res = tree_.insert(std::make_pair(key, value));
        if (res.second)
            return NO_VALUE;
        else
            return value;
